...
from core.database import BaseKaoyan

class KaoyanInfo(BaseKaoyan):
    """考研信息表"""
    __tablename__ = "kaoyan_info"
    ...
    degree_type = Column(Integer, nullable=True, comment="学位类型: 1-学硕, 2-专硕")
    study_type = Column(Integer, nullable=True, comment="学习方式: 1-全日制, 2-非全日制")
    ...
    def __repr__(self):
        return f"<KaoyanInfo(id={self.id}, title={self.title}, source={self.source})>"
    ...
    @property
    def degree_type_text(self):
        """获取学位类型文本"""
        if not self.degree_type:
            return ""
        return "学硕" if self.degree_type == 1 else "专硕"
    
    @property
    def study_type_text(self):
        """获取学习方式文本"""
        if not self.study_type:
            return ""
        return "全日制" if self.study_type == 1 else "非全日制"
    
class KaoyanCrawlerConfig(BaseKaoyan):
    """考研爬虫配置表"""
    __tablename__ = "kaoyan_crawler_config"
    ...
    def __repr__(self):
        return f"<KaoyanCrawlerConfig(id={self.id}, name={self.name}, url={self.url})>"
...
class KaoyanCrawlerLog(BaseKaoyan):
    """考研爬虫日志表"""
    __tablename__ = "kaoyan_crawler_logs"
    ...
    def __repr__(self):
        return f"<KaoyanCrawlerLog(id={self.id}, config_id={self.config_id}, status={self.status})>"
