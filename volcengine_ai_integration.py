...
class VolcEngineConfig:
    ...
    def from_env(cls) -> 'VolcEngineConfig':
        ...
        return cls(
            ...
            model=os.getenv('VOLCENGINE_MODEL', 'deepseek-chat'),
        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    logger.info(f"✅ AI分析成功: {task_type}, 耗时: {response_time:.2f}秒")
                    return result
                    
                elif response.status == 429:
                    # 限流，等待后重试
                    logger.warning("🔄 被限流，等待后重试...")
                    await asyncio.sleep(5)
                    return await self._call_volcengine_ai(task_type, text, context)
...
"""
    ...
    def _parse_response(self, task_type: str, response: Dict, original_text: str, context: Dict) -> Dict:
        ...
        try:
            ...
            # 构造兜底结果
            return self._construct_fallback_result(
                task_type,
                original_text,
                context,
                str(e)
    ...
    def _construct_fallback_result(self, task_type: str, text: str, context: Dict, error: str) -> Dict:
        """构造兜底结果"""
        ...
        if task_type == "analyze_demand":
            return {
                ...
                'urgency': 1,
                'entities': {},
                'ai_recommendations': ['AI服务调用失败，请检查配置'],
                'suggested_websites': [],
                'monitoring_strategy': {'frequency': 60, 'priority': 'low'},
                'personalized_advice': ['系统故障，请联系技术支持'],
                'task_type': task_type,
                'ai_model': self.config.model,
                'provider': 'volcengine_coding_plan',
                'response_timestamp': datetime.now().isoformat(),
...
class VolcEngineEnhancedCrawlerManager:
    ...
    def __init__(self, api_key: str):
        self.config = VolcEngineConfig(api_key=api_key)
    ...
    async def analyze_student_demand(self, student_id: int, raw_demand: str, student_profile: Dict) -> Dict:
        ...
        try:
            ...
            return self._get_fallback_analysis(raw_demand, student_profile)
    ...
    async def generate_smart_crawler_config(self, demand_analysis: Dict, student_id: int) -> Dict:
        ...
        try:
            ...
            return self._get_fallback_crawler_config(demand_analysis, student_id)
    ...
    def _get_fallback_analysis(self, raw_demand: str, student_profile: Dict) -> Dict:
        """获取兜底分析结果"""
        return {
            'success': False,
            'intent': 'unknown',
            ...
            'urgency': 1,
            'entities': {},
            'ai_recommendations': ['AI服务不可用，请检查配置'],
            'suggested_websites': [],
            'monitoring_strategy': {'frequency': 60, 'priority': 'low'},
            'personalized_advice': ['系统故障，请联系技术支持'],
            'fallback': True
    ...
    def _get_fallback_crawler_config(self, demand_analysis: Dict, student_id: int) -> Dict:
        """获取兜底爬虫配置"""
        return {
            'student_id': student_id,
            'timestamp': datetime.now().isoformat(),
...
async def demo_volcengine_integration():
    ...
    for test in test_cases:
        ...
        print(f"🎯 AI识别意图: {mock_analysis['intent']}")
        print(f"📊 置信度: {mock_analysis['confidence']:.2f}")
        print(f"🚨 紧急度: {mock_analysis['urgency']}/5")
        print(f"📍 实体: {mock_analysis['entities']}")
    ...
    config = {
        ...
        "crawler": {
            ...
            "priority": "high",
            "timeout": 30,
            "websites": ["https://yz.chsi.com.cn/", "https://www.pku.edu.cn/"],
            "filters": {
                "provinces": ["北京"],
                "schools": ["北京大学"],
                "majors": ["计算机"]
    ...
    print(f"🛠️ 生成的爬虫配置:")
    print(f"  频率: {config['crawler']['frequency']} 分钟")
    print(f"  优先级: {config['crawler']['priority']}")
    print(f"  监控网站数: {len(config['crawler']['websites'])}")
